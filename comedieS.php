	...
	<div class="container">
	   <div id="content" class="row">
		...
		<?php
			...
			$reponse = $bdd->query("SELECT * FROM films WHERE categorie='ComedieS' ORDER BY titre ASC LIMIT ".(($depart-1)*$nombre_max_page).",$nombre_max_page" );
...
<?php
    ...
    <div id="page_navigation"> 
		Page :
		<?php
			$total_tab = $bdd->query("SELECT COUNT(*) AS total FROM films WHERE categorie='ComedieS' ORDER BY titre ASC");
			...
			for($i = 1; $i <= $nombreDePages; $i++){
				echo " <a href=\"ComedieS.php?p=$i\"> $i - </a>";
			}
		?>
	</div>
</div>
